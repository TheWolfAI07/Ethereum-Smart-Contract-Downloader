...
var ContractForm = ({ onContractsFound, config, onShowNotification, backendStatus }) => {
  const [formData, setFormData] = useState({
    url: config?.url || 'https://mainnet.infura.io/v3/YOUR_API_KEY',
    blocks: config?.blocks || '*',
    api: config?.api || 'YOUR_ETHERSCAN_API_KEY',
  });
...
var validateForm = () => {
  ...
  if (!formData.url.trim()) {
    ...
    } else if (formData.api === 'YOUR_ETHERSCAN_API_KEY') {
